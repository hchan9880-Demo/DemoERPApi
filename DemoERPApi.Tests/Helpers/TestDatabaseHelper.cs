...
public class TestDatabaseHelper
{
    ...
    IF NOT EXISTS (SELECT 1 FROM Customers WHERE CRMCustomerID = 'CRM100')
        ...
        (CRMCustomerID, FirstName, LastName, Email, Phone, IsDeleted)
        VALUES 
        ('CRM100', 'Michael', 'Test', 'michael@test.com', '6041111111', 0);

    IF NOT EXISTS (SELECT 1 FROM Customers WHERE CRMCustomerID = 'CRM101')
        INSERT INTO Customers 
        (CRMCustomerID, FirstName, LastName, Email, Phone, IsDeleted)
        VALUES 
        ('CRM101', 'Sarah', 'Test', 'sarah@test.com', '6042222222', 0);

    IF NOT EXISTS (SELECT 1 FROM Customers WHERE CRMCustomerID = 'CRM103')
        INSERT INTO Customers 
        (CRMCustomerID, FirstName, LastName, Email, Phone, IsDeleted)
        VALUES 
        ('CRM103', 'Owner', 'User', 'owner@test.com', '6043333333', 0);

    IF NOT EXISTS (SELECT 1 FROM Customers WHERE CRMCustomerID = 'CRM300')
        INSERT INTO Customers 
        (CRMCustomerID, FirstName, LastName, Email, Phone, IsDeleted)
        VALUES 
        ('CRM300', 'Other', 'User', 'other@test.com', '6044444444', 0);

    IF NOT EXISTS (SELECT 1 FROM Customers WHERE CRMCustomerID = 'CRM301')
        INSERT INTO Customers 
        (CRMCustomerID, FirstName, LastName, Email, Phone, IsDeleted)
        VALUES 
        ('CRM301', 'Owner2', 'User', 'owner2@test.com', '6045555555', 0);


    -- =========================
    -- USERS SECOND
    -- FK: Users.CustomerID -> Customers.CRMCustomerID
    -- =========================
...
BEGIN
    ...
    (
        'admin',
        '$2a$11$CVy0LpgX98DcfUgfKiQYweHyRX5zLg8f74NnPTNzbaB7dTX4dfdAu',
        'Customer',
        'CRM100',
        1
    );
...
BEGIN
    ...
    (
        'sarah',
        '$2a$11$CTGbjzKYsBFyA0qskjP.OboAY/0QQZKFEdgV4ArDK8d4cqn.WCiq',
        'Customer',
        'CRM101',
        1
    );
...
END;
    ...
    -- FK: CustomerAccess -> Customers + Users
    -- =========================
